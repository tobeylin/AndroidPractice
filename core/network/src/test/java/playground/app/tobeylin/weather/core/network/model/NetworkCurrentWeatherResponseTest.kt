...
class NetworkCurrentWeatherResponseTest {
    ...
    fun deserialize_currentWeatherResponse_allFieldsMapped() {
        ...
        assertEquals(null, result.wind.deg)
        assertEquals(1711296000L, result.dt)
        assertEquals(25.033, result.coord.lat, 0.001)
    }

    @Test
    fun deserialize_currentWeatherResponse_withWindDeg() {
        val jsonString = """
            {"coord":{"lon":121.5654,"lat":25.033},"weather":[{"id":802,"main":"Clouds","description":"scattered clouds","icon":"03d"}],"main":{"temp":28.5,"feels_like":31.2,"temp_min":27.0,"temp_max":30.0,"humidity":70},"wind":{"speed":3.5,"deg":180},"dt":1711296000,"name":"Taipei"}
        """.trimIndent()

        val result = json.decodeFromString<NetworkCurrentWeatherResponse>(jsonString)

        assertEquals(3.5, result.wind.speed, 0.001)
        assertEquals(180, result.wind.deg)
    }
